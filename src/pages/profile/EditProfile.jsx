...
const EditProfile = (props) => {
  ...
  const editUser = async (userData) => {
    ...
    try {
      await axios
        .patch(`${baseURL}/${localStorage.getItem(LOCAL_ITEM.ID)}`, userData)
        .then((res) => setProfile(res.data));
      setIsSuccess(true);
    } catch (error) {
      const customError = new Error();
    ...
    <div className="flex flex-col row-start-1 row-span-7 col-start-3 col-span-6 ml-5">
      ...
      <form id="form1" className="my-10 mr-10" onSubmit={handleSubmit}>
        <div className="items-center flex justify-between mt-8">
          ...
          <input
            className="px-3 inline-block float-right w-3/4 h-10 outline-none rounded-[4px] border border-solid border-[#cccccc]"
            type="text"
        ...
        <div className="items-start flex justify-between mt-8">
          ...
          <input
            className={
              phoneVisible
                ? "px-3 inline-block float-right w-3/4 h-10 outline-none bg-[#F3F4FF] rounded-[4px] border border-solid border-[#cccccc]"
                : "px-3 inline-block float-right w-3/4 h-10 outline-none bg-[#ffff] rounded-[4px] border border-solid border-[#cccccc]"
            }
            type="text"
        ...
        <div className="items-start flex justify-between mt-8">
          ...
          <input
            className="px-3 inline-block float-right w-3/4 h-10 outline-none bg-[#F3F4FF] rounded-[4px] border border-solid border-[#cccccc]"
            type="password"
            disabled
